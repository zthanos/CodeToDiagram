<template>
  ...
  >
    ...
    <div class="teams-list__header">
      <div class="teams-list__controls">
        <!-- Search input -->
        <div class="teams-list__search">
          <input
            v-model="localSearchQuery"
            type="text"
            class="teams-list__search-input"
            placeholder="Search teams..."
            :data-testid="'teams-search'"
            @input="handleSearchChange"
            aria-label="Search teams"
          />
          <div class="teams-list__search-icon">🔍</div>
        </div>

        <!-- Add new team button -->
        <button
          class="teams-list__add-btn"
          :disabled="readonly"
          @click="handleAddTeam"
    ...
    <!-- Results summary -->
    <div class="teams-list__summary" :data-testid="'teams-summary'">
      Showing {{ filteredItems.length }} of {{ (items || []).length }} teams
    </div>

    <!-- Teams grid -->
    <div class="teams-list__grid" :data-testid="'teams-grid'">
      <div
    ...
    >
      <div class="teams-list__empty-icon">👥</div>
      <h3 class="teams-list__empty-title">
        {{ (items || []).length === 0 ? 'No teams yet' : 'No matching teams' }}
      </h3>
      <p class="teams-list__empty-description">
        {{ (items || []).length === 0 
    ...
    <div 
      v-if="showTeamModal"
      class="teams-list__modal-overlay"
      @click="closeTeamModal"
      :data-testid="'team-modal'"
    >
      <div 
        class="teams-list__modal"
        @click.stop
      >
        <h3 class="teams-list__modal-title">
          {{ editingTeam ? 'Edit Team' : 'Add New Team' }}
        </h3>
        
        <form @submit.prevent="handleSaveTeam" class="teams-list__form">
          <!-- Team name -->
          <div class="teams-list__form-group">
            <label for="team-name" class="teams-list__form-label">Name *</label>
            <input
              id="team-name"
              v-model="teamForm.name"
              type="text"
              class="teams-list__form-input"
              :class="{ 'teams-list__form-input--error': teamFormErrors.name }"
              placeholder="Enter team name"
              required
              :data-testid="'team-name-input'"
            />
            <div v-if="teamFormErrors.name" class="teams-list__form-error">
              {{ teamFormErrors.name }}
            </div>
          </div>
          ...
          <!-- Team role -->
          <div class="teams-list__form-group">
            <label for="team-role" class="teams-list__form-label">Role *</label>
            <input
              id="team-role"
              v-model="teamForm.role"
              type="text"
              class="teams-list__form-input"
              :class="{ 'teams-list__form-input--error': teamFormErrors.role }"
              placeholder="Enter team role (e.g., Development, QA, Design)"
              required
              :data-testid="'team-role-input'"
            />
            <div v-if="teamFormErrors.role" class="teams-list__form-error">
              {{ teamFormErrors.role }}
            </div>
          </div>
          ...
          <!-- Team members -->
          <div class="teams-list__form-group">
            <label for="team-members" class="teams-list__form-label">Members</label>
            <div class="teams-list__members-input">
              <input
                v-model="newMember"
                type="text"
                class="teams-list__form-input"
              ...
              <button
                type="button"
                class="teams-list__add-member-btn"
                @click="addMember"
            ...
            <ul v-if="teamForm.members.length > 0" class="teams-list__members-list">
              <li
                v-for="(member, index) in teamForm.members"
                :key="index"
                class="teams-list__member-item"
              >
                <div class="teams-list__member-avatar">{{ getInitials(member) }}</div>
                <span class="teams-list__member-name">{{ member }}</span>
                <button
                  type="button"
                  class="teams-list__remove-member-btn"
          ...
          <!-- Team responsibilities -->
          <div class="teams-list__form-group">
            <label for="team-responsibilities" class="teams-list__form-label">Responsibilities</label>
            <div class="teams-list__responsibilities-input">
              <input
              ...
              <button
                type="button"
                class="teams-list__add-responsibility-btn"
            ...
            <ul v-if="teamForm.responsibilities.length > 0" class="teams-list__responsibilities-list">
              <li
                v-for="(responsibility, index) in teamForm.responsibilities"
                :key="index"
                class="teams-list__responsibility-item"
              >
                <span>{{ responsibility }}</span>
                <button
                  type="button"
                  class="teams-list__remove-responsibility-btn"
          ...
          <!-- Form actions -->
          <div class="teams-list__form-actions">
            <button
              type="submit"
              class="teams-list__form-btn teams-list__form-btn--primary"
            ...
            <button
              type="button"
              class="teams-list__form-btn teams-list__form-btn--secondary"
              @click="closeTeamModal"
...
.teams-list__search-input:focus {
  outline: none;
  border-color: #0366d6;
  box-shadow: 0 0 0 3px rgba(3, 102, 214, 0.1);
}

.teams-list__search-icon {
  position: absolute;
  left: 12px;
  top: 50%;
...
.teams-list__add-btn:hover:not(:disabled) {
  background: #218838;
}

.teams-list__add-btn:focus {
  outline: none;
  box-shadow: 0 0 0 3px rgba(40, 167, 69, 0.25);
}

.teams-list__add-btn:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
...
.teams-list__empty-icon {
  font-size: 48px;
  margin-bottom: 16px;
  opacity: 0.5;
}

.teams-list__empty-title {
  margin: 0 0 8px 0;
  font-size: 18px;
  font-weight: 600;
  color: #24292e;
...
.teams-list__empty-btn:hover {
  background: #218838;
}

/* Modal */
.teams-list__modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
...
.teams-list__form-group {
  display: flex;
  flex-direction: column;
  gap: 4px;
}

.teams-list__form-label {
  font-size: 14px;
  font-weight: 500;
  color: #24292e;
}

.teams-list__form-input {
  padding: 8px 12px;
  border: 1px solid #e1e5e9;
  border-radius: 6px;
  font-size: 14px;
...
.teams-list__form-input:focus {
  outline: none;
  border-color: #0366d6;
  box-shadow: 0 0 0 3px rgba(3, 102, 214, 0.1);
}

.teams-list__form-input--error {
  border-color: #d73a49;
}

.teams-list__form-error {
  font-size: 12px;
  color: #d73a49;
  margin-top: 4px;
}
...
/* Members input */
.teams-list__members-input,
.teams-list__responsibilities-input {
  display: flex;
  gap: 8px;
}

.teams-list__members-input .teams-list__form-input,
.teams-list__responsibilities-input .teams-list__form-input {
  flex: 1;
}

.teams-list__add-member-btn,
.teams-list__add-responsibility-btn {
  padding: 8px 16px;
  background: #0366d6;
  color: #ffffff;
  border: none;
...
.teams-list__add-member-btn:hover:not(:disabled),
.teams-list__add-responsibility-btn:hover:not(:disabled) {
  background: #0256cc;
}

.teams-list__add-member-btn:disabled,
.teams-list__add-responsibility-btn:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}

.teams-list__members-list,
.teams-list__responsibilities-list {
  margin: 8px 0 0 0;
  padding: 0;
  list-style: none;
  display: flex;
...
.teams-list__member-item,
.teams-list__responsibility-item {
  display: flex;
  align-items: center;
...
.teams-list__member-name {
  flex: 1;
  color: #24292e;
}

.teams-list__remove-member-btn,
.teams-list__remove-responsibility-btn {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 20px;
...
.teams-list__remove-member-btn:hover,
.teams-list__remove-responsibility-btn:hover {
  background: #d73a49;
  color: #ffffff;
}

/* Form actions */
.teams-list__form-actions {
  display: flex;
  gap: 8px;
  justify-content: flex-end;
  margin-top: 8px;
...
.teams-list__form-btn--primary {
  background: #28a745;
  border-color: #28a745;
  color: #ffffff;
}

.teams-list__form-btn--primary:hover:not(:disabled) {
  background: #218838;
  border-color: #218838;
}

.teams-list__form-btn--primary:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}

.teams-list__form-btn--secondary {
  background: #ffffff;
  color: #24292e;
}

.teams-list__form-btn--secondary:hover {
  background: #f6f8fa;
}
